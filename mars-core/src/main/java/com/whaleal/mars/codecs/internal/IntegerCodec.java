...
public class IntegerCodec implements Codec<Integer>, RepresentationConfigurable<Integer> {
    ...
    private IntegerCodec(final BsonType representation) {
    ...
    public void encode(final BsonWriter writer, final Integer value, final EncoderContext encoderContext) {
        switch (representation) {
            ...
            case STRING:
                ...
                break;
            default:
                throw new BsonInvalidOperationException("Cannot encode a  Integer to a " + representation);
        }
    }

    @Override
    public Integer decode(final BsonReader reader, final DecoderContext decoderContext) {
        Integer value = NumberCodecHelper.decodeInt(reader);
        if (value < -Integer.MAX_VALUE || value > Integer.MAX_VALUE) {
            throw new BsonInvalidOperationException(format("%s can not be converted into a Integer.", value));
        }
        return value;
    }
    ...
    @Override
    public Codec<Integer> withRepresentation(BsonType bsonType) {
        if (representation != BsonType.STRING && representation != INT32 && representation != INT64 && representation != DOUBLE && representation != DECIMAL128) {
            throw new CodecConfigurationException(representation + " is not a supported representation for Integer");
        }
        return new IntegerCodec(bsonType);
    }
}
