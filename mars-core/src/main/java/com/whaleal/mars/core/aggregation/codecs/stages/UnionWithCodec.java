...
public class UnionWithCodec extends StageCodec<UnionWith> {
    ...
    protected void encodeStage(BsonWriter writer, UnionWith unionWith, EncoderContext encoderContext) {
        ...
        String collectionName = name != null ? name
                : getMapper().getEntityModel(unionWith.getCollectionType()).getCollectionName();

        ExpressionHelper.document(writer, () -> {
            ExpressionHelper.value(getMapper(), writer, "coll", collectionName, encoderContext);
            ExpressionHelper.value(getMapper(), writer, "pipeline", unionWith.getStages(), encoderContext);
        });
    }
}
