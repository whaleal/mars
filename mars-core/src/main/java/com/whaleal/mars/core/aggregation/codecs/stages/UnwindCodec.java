    ...
    protected void encodeStage(BsonWriter writer, Unwind value, EncoderContext encoderContext) {
        if (!value.optionsPresent()) {
            ...
            ExpressionHelper.document(writer, () -> {
                ExpressionHelper.expression(getMapper(), writer, "path", value.getPath(), encoderContext);
                ExpressionHelper.value(getMapper(), writer, "includeArrayIndex", value.getIncludeArrayIndex(), encoderContext);
                ExpressionHelper.value(getMapper(), writer, "preserveNullAndEmptyArrays", value.getPreserveNullAndEmptyArrays(), encoderContext);
            });
        }
    }
}
