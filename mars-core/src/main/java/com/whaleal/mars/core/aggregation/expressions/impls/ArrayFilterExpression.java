    ...
    public void encode(MongoMappingContext mapper, BsonWriter writer, EncoderContext encoderContext) {
        ExpressionHelper.document(writer, () -> {
            ExpressionHelper.document(writer, getOperation(), () -> {
                ExpressionHelper.expression(mapper, writer, "input", array, encoderContext);
                ExpressionHelper.expression(mapper, writer, "cond", conditional, encoderContext);
                ExpressionHelper.value(mapper, writer, "as", as, encoderContext);
            });
        });
    }
}
