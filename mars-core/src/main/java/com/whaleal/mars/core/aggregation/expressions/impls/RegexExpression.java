    ...
    public void encode(MongoMappingContext mapper, BsonWriter writer, EncoderContext encoderContext) {
        ExpressionHelper.document(writer, () -> {
            ExpressionHelper.document(writer, getOperation(), () -> {
                ExpressionHelper.expression(mapper, writer, "input", input, encoderContext);
                ExpressionHelper.value(mapper, writer, "regex", new BsonRegularExpression(regex), encoderContext);
                ExpressionHelper.value(mapper, writer, "options", options, encoderContext);
            });
        });
    }
